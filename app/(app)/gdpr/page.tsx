      ...
      <Card className="rounded-xl border border-border bg-card shadow-xs overflow-hidden">
        ...
        {isLoading ? (
          ...
          <>
            ...
            {/* Mobile (<md) : liste de cartes empilées, actions inaccessibles en scroll horizontal sur un tableau 5 colonnes */}
                  ...
                  <div key={req.id} className="p-4 space-y-3">
                    <div className="flex items-start justify-between gap-3">
                      ...
                      {req.type === "suppression" ? (
                        <Badge
                          variant="outline"
                          className="shrink-0 bg-red-500/10 text-red-700 dark:text-red-400 border-red-500/30 text-[11px] font-semibold gap-1"
                        >
                          <Trash2 className="h-3 w-3" /> Suppression
                        </Badge>
                      ) : (
                        <Badge
                          variant="outline"
                          className="shrink-0 bg-blue-500/10 text-blue-700 dark:text-blue-400 border-blue-500/30 text-[11px] font-semibold gap-1"
                        >
                          <Lock className="h-3 w-3" /> Anonymisation
                        </Badge>
                      )}
                    ...
                    <div className="flex items-center justify-between gap-2 text-[11px] text-muted-foreground">
                      ...
                      <span>{req.dateCreation}</span>
                    </div>

                    {(isPending || isFailed || req.rapportTechnique) && (
                      <div className="flex items-center gap-2 pt-1">
                        {(isPending || isFailed) && (
                          <Button
                            ...
                            disabled={isInProgress}
                            className="h-8 flex-1 text-[11px] font-semibold gap-1 rounded-md"
                          >
                            <KeyRound className="h-3 w-3" />
                            {isFailed ? "Relancer" : "Exécuter"}
                          </Button>
                        ...
                        {req.rapportTechnique && (
                          <Button
                            ...
                            size="sm"
                            onClick={() => toggleRow(req.id)}
                            className="h-8 flex-1 text-[11px] text-muted-foreground hover:text-foreground gap-1"
                          >
                            <span>Détails</span>
                            {isOpen ? (
                              <ChevronDown className="h-3 w-3" />
                    ...
                    {req.rapportTechnique && (
                      <Collapsible open={isOpen} onOpenChange={() => toggleRow(req.id)}>
                        <CollapsibleContent>
                          <div className="mt-1 rounded-xl border border-border bg-muted/20 p-3 space-y-3">
                            ...
                            <div className="rounded-lg bg-muted/50 p-2.5 font-mono text-[10px] text-muted-foreground break-all space-y-1">
                              <span className="font-semibold text-foreground block">
                                SHA-256 :
                              </span>
                              <span>{req.rapportTechnique.hashAudit}</span>
                              <span className="block text-[9px] text-muted-foreground/70 pt-1">
                                UTC : {req.rapportTechnique.horodatage}
                              </span>
                            </div>
                          </div>
                        </CollapsibleContent>
