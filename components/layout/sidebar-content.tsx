...
interface SidebarContentProps {
  /** En mode drawer mobile, le contenu est toujours "déployé" (pas de repli w-16). */
  collapsed?: boolean;
  /** Bouton replier/déployer visible uniquement en desktop. */
  showCollapseToggle?: boolean;
  /** Appelé après un clic sur un lien de navigation (ferme le drawer mobile). */
  onNavigate?: () => void;
}
...
export function SidebarContent({
  ...
  const { currentUser, toggleSidebar, setUserRole } = useAppStore();
  ...
  return (
    <div className="flex h-full flex-col select-none">
      ...
      {/* Collapse / Expand Toggle Button at Bottom (desktop uniquement) */}
      {showCollapseToggle && (
        <div className="p-2 border-t border-border flex items-center justify-between">
          ...
          <Button
            ...
            className={cn(
              ...
              collapsed ? "w-full justify-center" : "ml-auto px-2"
            )}
            aria-label={collapsed ? "Déployer la barre latérale" : "Rétracter la barre latérale"}
          >
            {collapsed ? (
